---
type: block
id: Ch06-S03-H03-B02
name: "Weak Convergence Example 6.3.3b"
name_zh: "弱收敛例子 6.3.3b"
keywords_en: "weak convergence; feature map"
aliases: ["Ch06-S03-H03-B02"]
status: placeholder
parent: Ch06-S03-H03
chapter: Ch06
cssclasses: ["lg-node", "lg-node--block"]
weight: 21
---

# Weak Convergence Example 6.3.3b

**弱收敛例子 6.3.3b** · example · 6.3.3b

⬆️ Part of [[Ch06-S03-H03 - Basic Properties Weak Convergence|Basic Properties: Weak Convergence]] · [[Ch06-S03-H03 - Basic Properties Weak Convergence|弱收敛：基本性质]]

The model case below shows that the hypothesis cannot be dropped:

$$
T e_k \;=\; \lambda_k e_k, \qquad
\sum_{k=1}^{\infty} |\lambda_k| \;<\; \infty,
 \qquad \lVert T \rVert_{\mathrm{tr}} = \sum_{k} |\lambda_k|.
$$

去掉条件后结论不再成立。

Used together with: [[Ch04-S02-H04-B02 - Proposition 4.2.4b|Proposition 4.2.4b]], [[Ch05-S03-H03-B02 - Example 5.3.3b|Example 5.3.3b]].

<!-- keywords:begin -->

Связи по `keywords_en` (weak convergence, feature map) — найдено в аннотациях `35 - Abstracts`. Число после × — вес ребра (сколько раз фраза встречается в этой области); блок создан алгоритмом, править его руками бессмысленно: перезапишется.

- [[Ch01-S03 - Main Results|weak convergence]] — секция `Ch01-S03`, 1 вхождение в корпусе
- [[Ch01-S03-H03 - Basic Properties Weak Convergence|weak convergence ×3]] — заголовок `Ch01-S03-H03`, 3 вхождения в корпусе
- [[Ch02-S02-H05 - Main Theorem Feature Map|feature map ×3]] — заголовок `Ch02-S02-H05`, 3 вхождения в корпусе
- [[Ch03-S02 - Core Theory|weak convergence]] — секция `Ch03-S02`, 1 вхождение в корпусе
- [[Ch03-S02-H01 - Definition Weak Convergence|weak convergence ×3]] — заголовок `Ch03-S02-H01`, 3 вхождения в корпусе
- [[Ch04-S01-H03 - Basic Properties Feature Map|feature map ×3]] — заголовок `Ch04-S01-H03`, 3 вхождения в корпусе
- [[Ch04-S04-H05 - Main Theorem Weak Convergence|weak convergence ×3]] — заголовок `Ch04-S04-H05`, 3 вхождения в корпусе
- [[Ch05-S04-H01 - Definition Feature Map|feature map ×2]] — заголовок `Ch05-S04-H01`, 2 вхождения в корпусе
- [[Ch07-S02 - Core Theory|feature map]] — секция `Ch07-S02`, 1 вхождение в корпусе
- [[Ch08-S02 - Core Theory|weak convergence]] — секция `Ch08-S02`, 1 вхождение в корпусе

**Вес по ключевым фразам: 21 · глава Ch04 (текст до первого `##` — связь с секцией, текст под `## Имя` — с заголовком)**
<!-- keywords:end -->
