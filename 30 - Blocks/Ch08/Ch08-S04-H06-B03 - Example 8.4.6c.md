---
type: block
id: Ch08-S04-H06-B03
name: "Lipschitz Continuity Example 8.4.6c"
name_zh: "利普希茨连续性例子 8.4.6c"
keywords_en: "lipschitz continuity; norm equivalence"
aliases: ["Ch08-S04-H06-B03"]
status: placeholder
parent: Ch08-S04-H06
chapter: Ch08
cssclasses: ["lg-node", "lg-node--block"]
weight: 24
---

# Lipschitz Continuity Example 8.4.6c

**利普希茨连续性例子 8.4.6c** · example · 8.4.6c

⬆️ Part of [[Ch08-S04-H06 - Proof and Consequences Lipschitz Continuity|Proof and Consequences: Lipschitz Continuity]] · [[Ch08-S04-H06 - Proof and Consequences Lipschitz Continuity|利普希茨连续性：证明与推论]]

The model case below shows that the hypothesis cannot be dropped:

$$
T e_k \;=\; \lambda_k e_k, \qquad
\sum_{k=1}^{\infty} |\lambda_k| \;<\; \infty,
 \qquad \lVert T \rVert_{\mathrm{tr}} = \sum_{k} |\lambda_k|.
$$

去掉条件后结论不再成立。

Used together with: [[Ch03-S01-H05-B02 - Identity 3.1.5b|Identity 3.1.5b]], [[Ch04-S02-H04-B01 - Example 4.2.4a|Example 4.2.4a]].

<!-- keywords:begin -->

Связи по `keywords_en` (lipschitz continuity, norm equivalence) — найдено в аннотациях `35 - Abstracts`. Число после × — вес ребра (сколько раз фраза встречается в этой области); блок создан алгоритмом, править его руками бессмысленно: перезапишется.

- [[Ch01-S02 - Core Theory|norm equivalence]] — секция `Ch01-S02`, 1 вхождение в корпусе
- [[Ch01-S02-H01 - Definition Dual Space|lipschitz continuity]] — заголовок `Ch01-S02-H01`, 1 вхождение в корпусе
- [[Ch01-S02-H04 - Key Lemma Norm Equivalence|norm equivalence ×3]] — заголовок `Ch01-S02-H04`, 3 вхождения в корпусе
- [[Ch02-S02-H02 - Notation Lipschitz Continuity|lipschitz continuity ×3]] — заголовок `Ch02-S02-H02`, 3 вхождения в корпусе
- [[Ch02-S03-H01 - Definition Sample Complexity|norm equivalence]] — заголовок `Ch02-S03-H01`, 1 вхождение в корпусе
- [[Ch03-S01 - Setup and Notation|norm equivalence]] — секция `Ch03-S01`, 1 вхождение в корпусе
- [[Ch03-S01-H02 - Notation Norm Equivalence|norm equivalence ×3]] — заголовок `Ch03-S01-H02`, 3 вхождения в корпусе
- [[Ch03-S02-H01 - Definition Weak Convergence|lipschitz continuity]] — заголовок `Ch03-S02-H01`, 1 вхождение в корпусе
- [[Ch03-S04-H06 - Proof and Consequences Lipschitz Continuity|lipschitz continuity ×3]] — заголовок `Ch03-S04-H06`, 3 вхождения в корпусе
- [[Ch04-S03-H01 - Definition Orthogonal Projection|norm equivalence]] — заголовок `Ch04-S03-H01`, 1 вхождение в корпусе
- [[Ch04-S03-H06 - Proof and Consequences Norm Equivalence|norm equivalence ×3]] — заголовок `Ch04-S03-H06`, 3 вхождения в корпусе
- [[Ch05-S03-H04 - Key Lemma Lipschitz Continuity|lipschitz continuity ×2]] — заголовок `Ch05-S03-H04`, 2 вхождения в корпусе
- [[Ch07-S02 - Core Theory|lipschitz continuity]] — секция `Ch07-S02`, 1 вхождение в корпусе

**Вес по ключевым фразам: 24 · глава Ch03 (текст до первого `##` — связь с секцией, текст под `## Имя` — с заголовком)**
<!-- keywords:end -->
