---
type: block
id: Ch01-S04-H01-B04
name: "Singular Values Example 1.4.1d"
name_zh: "奇异值例子 1.4.1d"
keywords_en: "singular values; norm equivalence"
aliases: ["Ch01-S04-H01-B04"]
status: placeholder
parent: Ch01-S04-H01
chapter: Ch01
cssclasses: ["lg-node", "lg-node--block"]
weight: 21
---

# Singular Values Example 1.4.1d

**奇异值例子 1.4.1d** · example · 1.4.1d

⬆️ Part of [[Ch01-S04-H01 - Definition Singular Values|Definition: Singular Values]] · [[Ch01-S04-H01 - Definition Singular Values|奇异值：定义]]

The model case below shows that the hypothesis cannot be dropped:

$$
T e_k \;=\; \lambda_k e_k, \qquad
\sum_{k=1}^{\infty} |\lambda_k| \;<\; \infty,
 \qquad \lVert T \rVert_{\mathrm{tr}} = \sum_{k} |\lambda_k|.
$$

去掉条件后结论不再成立。

Used together with: [[Ch06-S04-H01-B02 - Identity 6.4.1b|Identity 6.4.1b]], [[Ch07-S04-H06-B01 - Identity 7.4.6a|Identity 7.4.6a]].

<!-- keywords:begin -->

Связи по `keywords_en` (singular values, norm equivalence) — найдено в аннотациях `35 - Abstracts`. Число после × — вес ребра (сколько раз фраза встречается в этой области); блок создан алгоритмом, править его руками бессмысленно: перезапишется.

- [[Ch01-S02 - Core Theory|norm equivalence]] — секция `Ch01-S02`, 1 вхождение в корпусе
- [[Ch01-S02-H04 - Key Lemma Norm Equivalence|norm equivalence]] — заголовок `Ch01-S02-H04`, 1 вхождение в корпусе
- [[Ch01-S04 - Applications and Limits|singular values]] — секция `Ch01-S04`, 1 вхождение в корпусе
- [[Ch01-S04-H01 - Definition Singular Values|singular values ×2]] — заголовок `Ch01-S04-H01`, 2 вхождения в корпусе
- [[Ch02-S03-H01 - Definition Sample Complexity|norm equivalence]] — заголовок `Ch02-S03-H01`, 1 вхождение в корпусе
- [[Ch03-S01 - Setup and Notation|norm equivalence]] — секция `Ch03-S01`, 1 вхождение в корпусе
- [[Ch03-S01-H02 - Notation Norm Equivalence|norm equivalence ×2]] — заголовок `Ch03-S01-H02`, 2 вхождения в корпусе
- [[Ch03-S02-H05 - Main Theorem Singular Values|singular values ×2]] — заголовок `Ch03-S02-H05`, 2 вхождения в корпусе
- [[Ch04-S03-H01 - Definition Orthogonal Projection|norm equivalence]] — заголовок `Ch04-S03-H01`, 1 вхождение в корпусе
- [[Ch04-S03-H06 - Proof and Consequences Norm Equivalence|norm equivalence]] — заголовок `Ch04-S03-H06`, 1 вхождение в корпусе
- [[Ch05-S01-H03 - Basic Properties Singular Values|singular values]] — заголовок `Ch05-S01-H03`, 1 вхождение в корпусе
- [[Ch06-S02-H04 - Key Lemma Norm Equivalence|norm equivalence ×2]] — заголовок `Ch06-S02-H04`, 2 вхождения в корпусе
- [[Ch06-S04-H01 - Definition Singular Values|singular values]] — заголовок `Ch06-S04-H01`, 1 вхождение в корпусе
- [[Ch08-S01-H02 - Notation Norm Equivalence|norm equivalence]] — заголовок `Ch08-S01-H02`, 1 вхождение в корпусе
- [[Ch08-S02-H05 - Main Theorem Singular Values|singular values]] — заголовок `Ch08-S02-H05`, 1 вхождение в корпусе
- [[Ch09-S03-H06 - Proof and Consequences Norm Equivalence|norm equivalence ×2]] — заголовок `Ch09-S03-H06`, 2 вхождения в корпусе

**Вес по ключевым фразам: 21 · глава Ch01 (текст до первого `##` — связь с секцией, текст под `## Имя` — с заголовком)**
<!-- keywords:end -->
